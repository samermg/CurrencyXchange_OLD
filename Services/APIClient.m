...
-(void)ExecutePOST:(WebAPIServerResponse)block {
    ...
    if ([_FileURL isEqual:@""] || [_FileURL length]<10) {
        ...
        dispatch_queue_t dwnQueue = dispatch_queue_create("StartQ", NULL);
        dispatch_async(dwnQueue, ^ {
            [SGAPI call:self->_FileURL method:self->_HttpMethod headers:self->_HttpHeaderFields andParams:self->_AdditionalParameters res:^(NSDictionary * _Nullable json, NSError * _Nullable error) {
                if ([[json objectForKey:@"status"] isEqual:@"FAILED"]) {
                    error = [NSError errorWithDomain:@"com.example.CurrencyXchange"
                code:401
                userInfo:@{NSLocalizedDescriptionKey: @"Invalid Credentails",
                NSLocalizedFailureReasonErrorKey: [json objectForKey:@"details"],
                NSLocalizedRecoverySuggestionErrorKey: @"Please check the your credentails and try again."}];
                }
                block(json,error);
            }];
